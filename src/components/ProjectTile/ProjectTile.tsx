...
export default function ProjectTile({ projectKey }: { projectKey: string }) {
  ...
  const handleViewProjectClick = () => {
    navigate(`/project?key=${projectKey.replace(/ /g, "\\a")}`);
    setTimeout(() => {
  ...
  return (
    ...
    >
      ...
      <img
        ...
        src={img}
      />
      <span style={{ fontSize: "1.2rem", lineHeight: "1.5rem", marginBottom: '-0.5rem' }}>
        {projectKey}
      </span>
...
export function ProjectTileBanner({
  ...
  return (
    ...
    >
      ...
      <div style={{backgroundColor: color, width: '0.5rem', height: '0.5rem', clipPath: 'polygon(0 0, 100% 100%, 100% 0)', filter: 'brightness(50%)'}}/>
    </div>
  );
}
